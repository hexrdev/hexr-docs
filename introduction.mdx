...
Hexr identifies the **process**. A gateway can only identify the connection.

That distinction is the whole product, and it is testable rather than
rhetorical: a process that was never decorated with `@hexr_agent` writes no
context file, so the attestor emits no selectors for its PID, so SPIRE issues
it nothing. It runs perfectly well and it is **nobody** — no identity, and no
evidence it can sign.

That is not a policy you could misconfigure off. There is nothing for SPIRE to
match. [How identity is actually issued →](/architecture/per-process-identity)
...
<CardGroup cols={2}>
  ...
  <Card title="Tenant Isolation You Can Disprove" icon="key" href="/architecture/multi-tenancy">
    A separate trust domain **and a separate root CA** per tenant — not a label, not a `tenant_id` column. One tenant's credential is *unverifiable* to another, not merely denied. [Run the openssl commands yourself](/architecture/multi-tenancy); a CronJob re-proves it hourly.
  </Card>
  <Card title="Evidence That Proves Two Things" icon="circle-check" href="/security/evidence-verification">
    `intact` proves the sequence was never altered. `signature_valid_rows` proves **authorship** — signed inside the process, chaining to the SPIFFE root. They are independent, and the endpoint says so itself rather than letting you conflate them.
  </Card>
</CardGroup>
