...
```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#6366F1', 'primaryTextColor': '#E0E7FF', 'primaryBorderColor': '#818CF8', 'lineColor': '#A5B4FC', 'secondaryColor': '#1E1B4B', 'tertiaryColor': '#312E81', 'fontSize': '14px'}}}%%
flowchart LR
    CODE["<b>Your Code</b>\n@hexr_agent\n+ any framework"]
    
    BUILD["<b>hexr build</b>\nAST discovery\nDockerfile gen\nsidecar injection"]
    
    DEPLOY["<b>hexr deploy</b>\nK8s scheduling\nauto-registration"]

    subgraph RUNTIME["What Hexr gives you — automatically"]
        direction TB
        ID["🔐 Cryptographic Identity\nSPIFFE X.509 per process"]
        CREDS["🔑 Cloud Credentials\nAWS · GCP · Azure\nzero secrets in code"]
        MESH["🔒 mTLS Mesh\nEnvoy sidecar\nzero-trust networking"]
        OBS["📊 Full Observability\ntraces · metrics · costs\nper agent, per LLM call"]
        TOOLS["🔧 Secure Tool Access\nVault · Gateway · Sandbox\nOPA policy enforcement"]
        COMM["📡 Agent-to-Agent\nA2A protocol\ntask lifecycle · streaming"]
    end

    CODE -->|"one decorator"| BUILD
    BUILD -->|"one command"| DEPLOY
    DEPLOY --> RUNTIME

    style CODE fill:#4F46E5,stroke:#818CF8,stroke-width:2px,color:#E0E7FF
    style BUILD fill:#6366F1,stroke:#A5B4FC,stroke-width:2px,color:#E0E7FF
    style DEPLOY fill:#7C3AED,stroke:#C4B5FD,stroke-width:2px,color:#E0E7FF
    style RUNTIME fill:#1E1B4B,stroke:#6366F1,stroke-width:2px,color:#E0E7FF
    style ID fill:#312E81,stroke:#818CF8,color:#E0E7FF
    style CREDS fill:#312E81,stroke:#818CF8,color:#E0E7FF
    style MESH fill:#312E81,stroke:#818CF8,color:#E0E7FF
    style OBS fill:#312E81,stroke:#818CF8,color:#E0E7FF
    style TOOLS fill:#312E81,stroke:#818CF8,color:#E0E7FF
    style COMM fill:#312E81,stroke:#818CF8,color:#E0E7FF
```
