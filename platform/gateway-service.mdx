...
sequenceDiagram
    ...
    Agent->>GW: hexr_tool("aws_s3", action="list_buckets")
    GW->>GW: Resolve "aws_s3" → S3 endpoint
    GW->>CI: Exchange SPIFFE → AWS credentials
    CI-->>GW: Temporary AWS creds
    GW->>API: ListBuckets (with SigV4)
    API-->>GW: Response
    GW->>GW: Record telemetry span
    GW-->>Agent: Response
```
