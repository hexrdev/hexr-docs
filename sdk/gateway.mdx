...
sequenceDiagram
    ...
    participant GW as Hexr Gateway
    participant VAULT as Hexr Vault
    participant API as External API

    Agent->>GW: call_tool("brave_web_search", {query: "..."})
    Note over GW: Lookup tool definition
    Note over GW: Validate arguments against schema
    GW->>VAULT: Fetch API key for Brave Search
    VAULT-->>GW: API key (SPIFFE-authenticated)
    GW->>API: GET /search?q=... (with API key header)
    API-->>GW: Search results
    GW-->>Agent: ToolResult(success=true, result=...)
```
</Frame>
