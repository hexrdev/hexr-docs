...
```mermaid
sequenceDiagram
    participant A as Agent A<br/>(coordinator)
    participant E as Envoy Proxy
    participant OPA as OPA
    participant B as Agent B<br/>(researcher)

    A->>E: Send task to researcher (SPIFFE mTLS)
    E->>OPA: Can coordinator → researcher?
    OPA-->>E: ALLOW
    E->>B: Deliver task (mTLS)
    B-->>A: Result
```
